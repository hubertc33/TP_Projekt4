...
int main(int argc, char* args[]) {
    ...
    if (init(gWindow, gRenderer, SCREEN_WIDTH, SCREEN_HEIGHT) >= 0) {
        ...
        while (!quit) {
            ...
            float inter_theta = interpolate(prev_theta, quadrotor.GetState()[2], interpolation_factor);
...
int init(std::shared_ptr<SDL_Window>& gWindow, std::shared_ptr<SDL_Renderer>& gRenderer, int screenWidth, int screenHeight) {
    ...
    if (SDL_LoadWAV("D:\\TP\\Projekt4\\tp_projekt_4\\sound\\fly.wav", &flightSpec, &flightBuffer, &flightLength) == NULL ||
        SDL_LoadWAV("D:\\TP\\Projekt4\\tp_projekt_4\\sound\\stay.wav", &restSpec, &restBuffer, &restLength) == NULL) {
